...
import com.garganttua.api.commons.security.SecurityRandoms;
...
public enum KeyAlgorithm implements IKeyAlgorithm {
    ...
    public static KeyAlgorithm validateKeyAlgorithm(String input) throws IllegalArgumentException {
        ...
        return Arrays.stream(KeyAlgorithm.values())
    ...
    @Override
    public KeyRealmType getType() throws IllegalArgumentException {
        if (this.isSymetricAlgorithm()) {
            return KeyRealmType.SYMETRIC;
        } else if (this.isAsymetricAlgorithm()) {
            return KeyRealmType.ASYMETRIC;
        } else {
        	//Should never happen
    ...
    public SecretKey generateSymetricKey() throws IllegalArgumentException {
		...
		try {
			...
			keyGen.init(this.keySize, SecurityRandoms.secureRandom());
    ...
    public KeyPair generateAsymetricKey() throws IllegalArgumentException {
		...
		try {
			...
			if (this.algorithm.equals("EC")) {
				if( this.keySize == 512 )
					keyGen.initialize(new ECGenParameterSpec("secp521r1"), SecurityRandoms.secureRandom());
				else 
					keyGen.initialize(new ECGenParameterSpec("secp"+this.keySize+"r1"), SecurityRandoms.secureRandom());
			} else {
				keyGen.initialize(this.keySize, SecurityRandoms.secureRandom());
			}
    ...
    @Override
    public String geCipherName(EncryptionMode mode, EncryptionPaddingMode padding) throws IllegalArgumentException {
    ...
    @Override
	public String geSignatureName(SignatureAlgorithm signatureAlgorithm) {
        ...
        if( this.algorithm.equals("EC") )
        	algorithmName = "ECDSA";
        if( signatureAlgorithm == SignatureAlgorithm.HMAC_SHA1)
            return SignatureAlgorithm.HMAC_SHA1.getName();
        if( signatureAlgorithm == SignatureAlgorithm.HMAC_SHA224)
            return SignatureAlgorithm.HMAC_SHA224.getName();
        if( signatureAlgorithm == SignatureAlgorithm.HMAC_SHA256)
            return SignatureAlgorithm.HMAC_SHA256.getName();
        if( signatureAlgorithm == SignatureAlgorithm.HMAC_SHA384)
            return SignatureAlgorithm.HMAC_SHA384.getName();
        if( signatureAlgorithm == SignatureAlgorithm.HMAC_SHA512)
            return SignatureAlgorithm.HMAC_SHA512.getName();
