...
public class GGAPIDynamicDomainsRegistry implements IGGAPIDynamicDomainsRegistry {
	...
	private GGAPIDynamicDomain getDynamicDomainFromEntityClass(Class<?> clazz) throws GGAPIEngineException {
		...
		if( domain.equals(GGAPIKeyRealmEntity.domain) && this.keyManagerType == GGAPIKeyManagerType.mongo ) {
			db = GGAPIDao.mongo;
		}
		
		if( domain.equals(GGAPIToken.domain) && this.tokenProviderType == GGAPITokenProviderType.mongo ) {
			db = GGAPIDao.mongo;
		}
		...
		return new GGAPIDynamicDomain(domain, entityClass, dtoClass, db, ws, controller,
				business, event, repo, dao, allow_creation, allow_read_all, allow_read_one,
