...
@Service
@Slf4j
public class SpringOwnerVerifierFilter extends SpringHttpApiFilter {
	...
	protected HttpServletRequest doFilter(HttpServletRequest request, HttpServletResponse response) throws CoreException {
		...
		if( SpringAuthentication.class.isAssignableFrom(authentication.getClass()) ) {
			log.atDebug().log("Checking caller ownerId ["+caller.getOwnerId()+"] against authentication ownerId ["+((SpringAuthentication) authentication).getOwnerId()+"]");
			
			this.security.verifyOwner(caller, ((SpringAuthentication) authentication).getAuthentication());
		}
		return request;
	}
}
