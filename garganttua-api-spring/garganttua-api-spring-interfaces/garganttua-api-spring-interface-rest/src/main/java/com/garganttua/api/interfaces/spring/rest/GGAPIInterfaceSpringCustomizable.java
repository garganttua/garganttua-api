...
public class GGAPIInterfaceSpringCustomizable implements IGGAPICustomizableInterface {
  ...
  protected void createCustomMappings() throws NoSuchMethodException, SecurityException {
    ...
    Method customMethod = this.getClass().getMethod("customService", IGGAPICaller.class, Map.class, HttpServletRequest.class);
    for( IGGAPIServiceInfos custom: this.customServicesInfos ) {
      this.createMapping(custom.getPath(), customMethod, this, options, RequestMethod.resolve(GGAPIServiceMethodToHttpMethodBinder.fromServiceMethod(custom.getOperation())));
    }
  }
  
  protected void createMapping(String path, Method method, Object handler, RequestMappingInfo.BuilderConfiguration options, RequestMethod requestMethod) {
