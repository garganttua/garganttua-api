...
import com.garganttua.api.commons.security.injection.Login;
...
/**
 * Resolves a {@link Login}-annotated parameter via {@link LoginSupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { Login.class })
public class LoginElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new LoginSupplierBuilder(), isNullable(element));
    }
