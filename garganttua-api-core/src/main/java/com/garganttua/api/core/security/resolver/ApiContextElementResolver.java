...
import com.garganttua.api.commons.security.injection.ApiContext;
...
/**
 * Resolves a {@link ApiContext}-annotated parameter via {@link ApiSupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { ApiContext.class })
public class ApiContextElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new ApiSupplierBuilder(), isNullable(element));
    }
