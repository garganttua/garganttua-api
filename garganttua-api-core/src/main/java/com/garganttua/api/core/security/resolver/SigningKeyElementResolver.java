...
import com.garganttua.api.commons.security.injection.SigningKey;
...
/**
 * Resolves a {@link SigningKey}-annotated parameter via {@link DomainKeySupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { SigningKey.class })
public class SigningKeyElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new DomainKeySupplierBuilder(), isNullable(element));
    }
