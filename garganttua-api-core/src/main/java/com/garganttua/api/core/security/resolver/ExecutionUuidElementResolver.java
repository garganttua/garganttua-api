...
import com.garganttua.api.commons.security.injection.ExecutionUuid;
...
/**
 * Resolves a {@link ExecutionUuid}-annotated parameter via {@link ExecutionUuidSupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { ExecutionUuid.class })
public class ExecutionUuidElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new ExecutionUuidSupplierBuilder(), isNullable(element));
    }
