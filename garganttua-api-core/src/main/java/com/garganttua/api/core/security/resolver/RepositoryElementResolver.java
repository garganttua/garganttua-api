...
import com.garganttua.api.commons.security.injection.Repository;
...
/**
 * Resolves a {@link Repository}-annotated parameter via {@link RepositorySupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { Repository.class })
public class RepositoryElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new RepositorySupplierBuilder(), isNullable(element));
    }
