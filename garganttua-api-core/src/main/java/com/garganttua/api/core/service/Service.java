...
public class Service implements IService {
	...
	public IServiceResponse createEntity(ICaller caller, Object entity, Map<String, String> customParameters) {
		IServiceCommand command = (event) -> {
			event.setIn(entity);
			Object preparedEntity = this.factory.prepareNewEntity(customParameters, entity, EntityHelper.getUuid(entity), caller.getRequestedTenantId());
		...
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowCreation();}, command, customParameters, EntityOperation.createOne(this.domain.getDomain(), this.domain.getEntityClass()));
	}

	@Override
	public IServiceResponse getEntities(ICaller caller, ReadOutputMode mode, IPageable pageable, IFilter filter, ISort sort, Map<String, String> customParameters) {
		IServiceCommand command = (event) -> {
		...
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowReadAll();}, command, customParameters, EntityOperation.readAll(this.domain.getDomain(), this.domain.getEntityClass()));
	}
	...
	public IServiceResponse getEntity(ICaller caller, String uuid, Map<String, String> customParameters) {
		IServiceCommand command = (event) -> {
			...
			Object entity = this.factory.getEntityFromRepository(caller, customParameters, EntityIdentifier.UUID, uuid);
			event.setOut(entity);
			event.setCode(ServiceResponseCode.OK);
			return event;
		};
		
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowReadOne();}, command, customParameters, EntityOperation.readOne(this.domain.getDomain(), this.domain.getEntityClass()));
	}

	@Override
	public IServiceResponse updateEntity(ICaller caller, String uuid, Object entity, Map<String, String> customParameters) {
		IServiceCommand command = (event) -> {
			event.setIn(entity);
		...
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowUpdateOne();}, command, customParameters, EntityOperation.updateOne(this.domain.getDomain(), this.domain.getEntityClass()));
	}
	...
	public IServiceResponse deleteEntity(ICaller caller, String uuid, Map<String, String> customParameters) {
		...
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowDeleteOne();}, command, customParameters, EntityOperation.deleteOne(this.domain.getDomain(), this.domain.getEntityClass()));
	}
	...
	public IServiceResponse deleteAll(ICaller caller, IFilter filter, Map<String, String> customParameters) {
		...
		return this.executeServiceCommand(caller, () -> {return this.domain.isAllowDeleteAll();}, command, customParameters, EntityOperation.deleteAll(this.domain.getDomain(), this.domain.getEntityClass()));
	}

	@Override
	public IServiceResponse executeServiceCommand(ICaller caller, Allowed allowed, IServiceCommand command, Map<String, String> customParameters, EntityOperation operation) {
