...
/**
 * Resolves a {@link DomainContext}-annotated parameter via {@link DomainSupplierBuilder} — the
 * declarative path to the supplier that {@code .withParam(...)} wires manually.
 */
@Reflected
@Resolver(annotations = { DomainContext.class })
public class DomainContextElementResolver implements IElementResolver {

    @Override
    public Resolved resolve(IClass<?> elementType, IAnnotatedElement element) throws DiException {
        return new Resolved(true, elementType, new DomainSupplierBuilder(), isNullable(element));
    }
