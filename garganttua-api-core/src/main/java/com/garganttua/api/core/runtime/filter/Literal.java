...
public class Literal implements IFilter {
	...
	public static void validate(Literal literal) throws LiteralException {
		...
		if (literal.name != null && !literal.name.startsWith(OPERATOR_PREFIX)) {
			throw new LiteralException("Invalid literal name, should start with $");
		}
			...
			switch (literal.name) {
				...
				case OPERATOR_REGEX:
					...
					if (literal.literals != null && !literal.literals.isEmpty()) {
						throw new LiteralException(
								"Literal of type " + literal.name + " does not accept sub literals");
					}
					break;
				case OPERATOR_IN:
				case OPERATOR_NOT_IN:
					...
					if (literal.literals == null || literal.literals.size() < 1) {
						throw new LiteralException(
								"Literal of type " + literal.name + " needs at least 1 sub literals");
					}
					for (Literal sub : literal.literals) {
						if (sub.name != null && !sub.name.isEmpty()) {
							throw new LiteralException(
									"Literal of type " + literal.name + " cannot have sub literal with a name");
						}
						if (sub.value == null) {
							throw new LiteralException(
									"Literal of type " + literal.name + " cannot have sub literal without value");
						}
						if (sub.literals != null && sub.literals.size() > 0) {
							throw new LiteralException(
									"Literal of type " + literal.name + " cannot have sub literals with sub literals");
						}
					}
				...
				case OPERATOR_TEXT:
					...
					for (Literal sub : literal.literals) {
						if (sub.name != null && !sub.name.isEmpty() && !sub.name.equals(OPERATOR_FIELD)) {
							throw new LiteralException(
									"Literal of type " + literal.name + " cannot have sub literal other than $field");
						}
						if (sub.value == null) {
							throw new LiteralException(
									"Literal of type " + literal.name + " cannot have sub literal without value");
				...
				case OPERATOR_NOR:
					...
					if (literal.literals == null || literal.literals.size() < 2) {
						throw new LiteralException(
								"Literal of type " + literal.name + " needs at least 2 sub literals");
					}
					break;
				case OPERATOR_FIELD:
					if (literal.value == null) {
					...
					if (literal.literals != null && literal.literals.size() > 1) {
						throw new LiteralException(
								"Literal of type " + literal.name + " needs 0 or 1 sub literals");
					}
					if (literal.literals != null && literal.literals.size() == 1 && !isFinal(literal.literals.get(0))) {
						throw new LiteralException("Literal of type " + literal.name
					...
					break;
				default:
					throw new LiteralException("Invalid literal name " + literal.name);
			}
		}
